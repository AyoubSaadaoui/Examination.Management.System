			...
			<?php if($row && $answered_test_row && $answered_test_row->submitted):?>
				...
				<div style="font-family: roboto;font-size: 18px;max-width: 1000px;margin: auto;">
					<table class="table">
						...
						<tr>
						 	<th class="th">Class: </th><td><?=$row->class->class?></td>
						 	<th class="th">Student: </th><td><?=$student_row->firstname?> <?=$student_row->lastname?></td>
					 	</tr>
						<tr>
							<th class="th">Created by:</th><td><?=esc($row->user->firstname)?> <?=esc($row->user->lastname)?></td>
							<th class="th">Date Created:</th><td><?=get_date($row->date)?></td>
						</tr>
						<tr>
					...
					<br>
						...
						<?php if(isset($questions) && is_array($questions)):?>
							...
							<?php foreach($questions as $question): $num++?>
								...
								<div>
									<table style="  border: 2px solid; border-color: white; border-collapse: collapse; width:100%; color:black; background-color:white;">
										<tr >
											<!--  -->
											<th style="text-align: left;"><span>Question #<?=$num?></span></th>
											<?php if($question->comment): ?>
												<th style="text-align: right;"><p><?=esc($question->comment)?></p></th>
											<?php endif;?>
										</tr>
									</table>
									  ...
									  <div style="background-color: #eee;padding: 10px;">
								    	...
								    	?>
							    				...
							    				<div style="font-size: 20px;float:right;">
							    					<?=($mymark == 1) ?
														'<img src="http://localhost/school/public/assets/correct.png"  id = "right">'
														:'<img src="http://localhost/school/public/assets/wrong.png" id = "right" >'
													?>
							    				</div>
